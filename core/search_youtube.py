...
class YoutubeSearch:
    ...
    def fetch_video_info(self, videoId):
        try:
            ...
            logger.error(
                f"[YOUTUBE SEARCH] Failed to fetch information of videos whose id {videoId}: {e}"
            )
            traceback.print_exc()
        return {}
    ...
    def search(self, query, before=PUBLISHED_FROM, after=PUBLISHED_TO, region_code=""):
        ...
        try:
                    ...
                    results.append(
                        {
                            ...
                            "vietsub": (
                                self.translator.translate_using_gemini(
                                    text=info.get("snippet", {}).get("description", "")
                                )
            ...
            logger.error(
                f"[YOUTUBE SEARCH] Failed to fetch list of videos with query {query}: {e}"
            )
            traceback.print_exc()
